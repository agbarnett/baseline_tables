...
infile = c('extracted','extracted_trialstreamer','extracted_validation') # see 1_which_data_source.R
outfile = c('analysis_ready','analysis_ready_trialstreamer','analysis_ready_validation') # see 1_which_data_source.R
which_source = 3
source = sources[which_source]
infile = paste('data/', infile[which_source], '.RData', sep='')
outfile = paste('data/', outfile[which_source], '.RData', sep='')
...
excluded_counts = group_by(excluded_counts, reason) %>%
  ...
  arrange(-count)
## save for analysis ##
save(excluded, excluded_counts, pvalues, table_data, design, file=outfile)
