...
#
ftab = flextable(tab) %>%
  add_header_row(values=c(' ', rep('Algorithm', 2))) %>%
...
bin = binom.confint(x=x, n=x, method='exact') %>%
  mutate(lower = roundz(lower*100,1),
         upper = roundz(upper*100,0))
```

The two "Missing" for the hand algorithm were tables that were excluded, although neither had p-values so would have agreed with the algorithm. 

The exact 95% confidence interval for the percentage of matches is `r bin$lower`% to `r bin$upper`%. Hence the algorithm has a high accuracy for delineating if p-values were included in baseline tables.
