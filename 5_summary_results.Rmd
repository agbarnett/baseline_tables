...
str_detect(pmcid, pattern='Correlated') ~ 'No',
str_detect(pmcid, pattern='3arms') ~ 'No'
  ),
...
str_detect(pmcid, pattern='Correlated0\\.2') ~ 'Correlated continuous 0.2',
str_detect(pmcid, pattern='Correlated0\\.6') ~ 'Correlated continuous 0.6',
str_detect(pmcid, pattern='3arms') ~ 'Three arms'
  ))
}
...
We used a threshold of `r paste(threshold, collapse=' and ', sep='')` for the trial-level probability of an additional precision.
...
### Testing that p-values follow a uniform distribution
...
### Testing that p-values follow a uniform distribution, excluding categorical results

The tests above use all available p-values, but the original uniform test can only use continuous summary statistics. Hence, below we re-run the tests after excluding the categorical summary statistics.

```{r}
# test uniform distribution for p-values
unif_pvals_continuous_only = filter(for_model, statistic == 'continuous') %>%
  group_by(h0, sgroup, pmcid) %>%
  summarise(n=n(), p = kolmogorov.unif.test(p, nrepl=2000, k=0)$p.value) %>%
  filter(n >= 3) # Must be at least three statistics

# summary stats
unif_tab_continuous_only = mutate(unif_pvals_continuous_only, sig = p < 0.05) %>%
  summarise(below = sum(sig), n=n()) %>%
  mutate(prop = roundz(100*below/n, 1),
         cell = paste(below, ' / ', n, ' (', prop, ')', sep='')) %>%
  arrange(h0, sgroup, below) 
renamed = select(unif_tab_continuous_only, h0, sgroup, cell) %>%
  rename('H0' = 'h0',
    'Simulation' = 'sgroup',
    'n (%)' = 'cell')
# nice table  
...
The cells show the number of positive tests and the number of simulations. For some simulated scenarios, the uniform test could not be run due to a lack of observations, hence the varying denominator. 