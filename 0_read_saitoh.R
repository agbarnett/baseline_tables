# 0_read_saitoh.R
# read in the baseline table data for the problematic trials by Saitoh
# June 2022
library(janitor)
library(dplyr)
library(readxl)
...
n_trials = 10
trials = paste('saitoh', 1:n_trials, sep='')
all = NULL
for (t in trials){
	raw = read_excel('data/retracted_papers_saitoh.xlsx', sheet=t) %>%
		clean_names() %>%
	  mutate(trial = t)
...
# add sample size
sample_sizes = read_excel('data/retracted_papers_saitoh.xlsx', sheet='sample_sizes') %>%
  clean_names()
...
save(date_downloaded, table_data, file='data/saitoh.RData')
